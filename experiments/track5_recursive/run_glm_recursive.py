"""
Track 5 GLM Recursive Trials — Adaptive policy optimization with live GLM-4.7 API.

Same as Claude recursive trials, but uses GLMReasoningClient.
Tests whether penalty ratchet and Goodhart dynamics reproduce across models.

Uses GLM baseline results for comparison.
API calls: 5 x 16 x 4 = 320 GLM calls.
"""
...
STEPS_PER_TASK = 4
N_ITERATIONS = 5
RESULTS_DIR = PROJECT_ROOT / "experiments" / "results" / "glm_recursive"

# GLM baseline results for comparison
GLM_BASELINE_PATH = (
...
def load_glm_baseline_scores() -> dict:
    """Load baseline quality scores from GLM baseline results."""
...
def run_glm_recursive_trials(n_iterations: int = N_ITERATIONS) -> dict:
    """Run the full recursive learning loop with live GLM API."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    print(f"Track 5 GLM Recursive Trials — {n_iterations} iterations")
    print(f"Timestamp: {timestamp}")
    print()

    # Load existing GLM baseline scores
    print("Loading GLM baseline scores...")
    ...
    out_path = out_dir / "glm_recursive_trials.json"
    ...
    # Print final summary
    print(f"\n{'='*60}")
    print(f"  TRACK 5 GLM COMPLETE — {n_iterations} iterations")
    print(f"{'='*60}")
    print(f"  GLM Baseline mean quality: {baseline_mean:.4f}")
