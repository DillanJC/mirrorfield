"""
Track 4 Mirror Switched — Claude with reflective (non-directive) interventions.

Same pipeline as run_live_switched.py, but uses MIRROR_INTERVENTIONS instead
of DEFAULT_INTERVENTIONS. Mirror interventions describe the model's geometric
state ("You are between two clusters") rather than directing behavior
("Identify the tension...").

Hypothesis: directive interventions harm quality because they impose an
external agenda. Reflective interventions should inform without disrupting
the model's natural reasoning trajectory.

API calls: 16 tasks x 4 steps = 64 Claude calls.
"""
...
# Configuration
STEPS_PER_TASK = 4
RESULTS_DIR = PROJECT_ROOT / "experiments" / "results" / "mirror_switched"


def run_mirror_switched() -> dict:
    """Run live switched reasoning with mirror (reflective) interventions."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    print(f"Track 4 Mirror Switched (Claude) — timestamp={timestamp}")
    print("Intervention style: REFLECTIVE (mirror)")
    ...
    # Key difference: pass MIRROR_INTERVENTIONS instead of defaults
    engine = SwitchEngine(
        reference, k=k,
    ...
    print(f"Running {len(tasks)} tasks ({STEPS_PER_TASK} steps each) with mirror switches...\n")
    ...
    out_path = out_dir / "mirror_switched_results.json"

    with open(out_path, 'w') as f:
        json.dump(output, f, indent=2)

    print(f"\nMirror switched run complete. Mean quality: {summary['mean_quality']:.3f}")
