"""
Track 4 Live Switched — Run tasks with live Claude API + geometric interventions.

Same as live baseline, but after each reasoning step, SwitchEngine.evaluate()
checks for intervention. If triggered, the intervention text is injected as a
real meta-instruction into the next Claude API call.

Key difference from simulation: Claude genuinely responds to interventions,
producing different reasoning trajectories.
...
# Configuration
STEPS_PER_TASK = 4
RESULTS_DIR = PROJECT_ROOT / "experiments" / "results" / "track4_live_switched"


def run_live_switched() -> dict:
    """Run live switched reasoning across all tasks."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    print(f"Track 4 Live Switched — timestamp={timestamp}")
    ...
    for task in tasks:
        ...
        for step_idx in range(STEPS_PER_TASK):
            # Generate step via Claude API (with intervention from previous step)
    ...
    print(f"\nLive switched run complete. Mean quality: {summary['mean_quality']:.3f}")
