"""
Track 4 GLM Mirror Switched — GLM-4.7 with reflective (non-directive) interventions.

Same pipeline as run_glm_switched.py, but uses MIRROR_INTERVENTIONS instead
of DEFAULT_INTERVENTIONS. Tests whether the mirror approach transfers across
models.
...
# Configuration
STEPS_PER_TASK = 4
RESULTS_DIR = PROJECT_ROOT / "experiments" / "results" / "glm_mirror_switched"


def run_glm_mirror_switched() -> dict:
    """Run live switched reasoning with mirror interventions using GLM-4.7."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    print(f"Track 4 GLM Mirror Switched — timestamp={timestamp}")
    ...
    out_path = out_dir / "glm_mirror_switched_results.json"

    with open(out_path, 'w') as f:
        json.dump(output, f, indent=2)

    print(f"\nGLM mirror switched run complete. Mean quality: {summary['mean_quality']:.3f}")
