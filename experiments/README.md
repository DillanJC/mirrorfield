...
| `sycophancy/` (C1) | does the model abandon a CORRECT answer under user pushback; does its confidence expose the cave? | **§4v** (corrected via clean re-ask control) — stable to neutral re-ask (1.5%); ~+40pt flips under any disagreement; coarsely graded by doubt (flat within explicit pushback); signal doesn't drop to flag it |
