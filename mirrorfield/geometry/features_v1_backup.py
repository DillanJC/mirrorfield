"""
Geometry Features — Native k-NN Statistics (Validated +6.4% Improvement)

Implements the 7 k-NN features proven to improve boundary detection.

Design Principles:
- Reference-only computation (ParallaxChain): queries never pollute reference graph
...
def compute_knn_features(
    ...
    """
    Compute 7 k-NN geometric features (reference-only, ParallaxChain compliant).
...
def compute_centroid_anchor(
    ...
    """
    Compute distance from each query to reference centroid (Centroid Anchor / CA).
...
def detect_dark_rivers(
    ...
    Args:
        local_curvature: Jitter Curvature values (N,)
        ridge_proximity: Separatrix Density values (N,)
...
def detect_observer_mode(
    ...
    """
    Detect Observer Mode: low curvature + close to reference centroid.

    Observer mode indicates regions where:
    - Model is uncertain (low curvature)
    - But data seems "normal" (close to reference centroid)

    Args:
        local_curvature: Jitter Curvature values (N,)
        centroid_anchor: Distances to reference centroid (N,)
...
def batch_invariance_test(
    ...
    """
    Test ParallaxChain guarantee: batch permutation invariance.
